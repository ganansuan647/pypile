...
class ArrangeModel(BaseModel):
    ...
    simu_pile_coordinates: List[PileCoordinate] = Field(..., description="模拟桩坐标列表")
    ...
    def validate_coordinates_count(cls, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if len(pile_coordinates) != pnum:
            raise ValueError(f"非模拟桩坐标数量({len(pile_coordinates)})与PNUM({pnum})不符")
        
        if len(simu_pile_coordinates) != snum:
            raise ValueError(f"模拟桩坐标数量({len(simu_pile_coordinates)})与SNUM({snum})不符")
...
class ArrangeInfoModel(BaseModel):
    ...
    def parse_input(cls, data: Any) -> Dict[str, Any]:
        ...
        if len(values) < 2 + (pnum + snum) * 2:
            raise ValueError(f"桩布置信息不完整，需要{2 + (pnum + snum) * 2}个值，但只找到{len(values)}个")
        
        # 构建非模拟桩坐标列表
        pile_coordinates = []
        for i in range(pnum):
            idx = 2 + i * 2
            pile_coordinates.append(PileCoordinate(
                X=values[idx],
                Y=values[idx+1]
            ))
        ...
        for i in range(snum):
            idx = 2 + pnum * 2 + i * 2
            simu_pile_coordinates.append(PileCoordinate(
                X=values[idx],
                Y=values[idx+1]
            ))
