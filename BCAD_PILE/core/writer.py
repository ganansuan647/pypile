...
def write_output_file(filename, jctr, ino, force, so, pile_results):
    ...
    with open(filename, "w") as f:
        ...
        f.write(
            f"{' ' * 16}Movement in the direction of Y axis : UY= {force[1]:.4E} (m)\n"
        )
        f.write(
            f"{' ' * 16}Movement in the direction of Z axis : UZ= {force[2]:.4E} (m)\n"
        )
        f.write(
            f"{' ' * 16}Rotational angle  around X axis :     SX= {force[3]:.4E} (rad)\n"
        )
        f.write(
            f"{' ' * 16}Rotational angle around Y axis :      SY= {force[4]:.4E} (rad)\n"
        )
        f.write(
            f"{' ' * 16}Rotational angle around Z axis :      SZ= {force[5]:.4E} (rad)\n\n"
        )
        ...
        for pile in pile_results:
            ...
            f.write(
                f"{' ' * 12}Displacements and internal forces at the top of pile:\n\n"
            )
            f.write(
                f"{' ' * 15}UX= {pile['top_displacement'][0]:.4E} (m){' ' * 9}NX= {pile['top_force'][0]:.4E} (t)\n"
            )
            f.write(
                f"{' ' * 15}UY= {pile['top_displacement'][1]:.4E} (m){' ' * 9}NY= {pile['top_force'][1]:.4E} (t)\n"
            )
            f.write(
                f"{' ' * 15}UZ= {pile['top_displacement'][2]:.4E} (m){' ' * 9}NZ= {pile['top_force'][2]:.4E} (t)\n"
            )
            f.write(
                f"{' ' * 15}SX= {pile['top_displacement'][3]:.4E} (rad){' ' * 7}MX= {pile['top_force'][3]:.4E} (t*m)\n"
            )
            f.write(
                f"{' ' * 15}SY= {pile['top_displacement'][4]:.4E} (rad){' ' * 7}MY= {pile['top_force'][4]:.4E} (t*m)\n"
            )
            f.write(
                f"{' ' * 15}SZ= {pile['top_displacement'][5]:.4E} (rad){' ' * 7}MZ= {pile['top_force'][5]:.4E} (t*m)\n\n"
            )
            ...
            for i in range(ig):
                f.write(
                    f"{' ' * 7}{pile['z_coordinates'][i]:14.4E}{pile['displacements_x'][i]:14.4E}{pile['displacements_y'][i]:14.4E}{pile['rotations_x'][i]:14.4E}{pile['rotations_y'][i]:14.4E}\n"
                )

            # Buried segments (below ground)
            for i in range(ig, num_points):
                f.write(
                    f"{' ' * 7}{pile['z_coordinates'][i]:14.4E}{pile['displacements_x'][i]:14.4E}{pile['displacements_y'][i]:14.4E}{pile['rotations_x'][i]:14.4E}{pile['rotations_y'][i]:14.4E}{pile['soil_stress_x'][i]:14.4E}{pile['soil_stress_y'][i]:14.4E}\n"
                )
            ...
            for i in range(num_points):
                f.write(
                    f"{' ' * 7}{pile['z_coordinates'][i]:16.4E}{pile['shear_x'][i]:16.4E}{pile['shear_y'][i]:16.4E}{pile['axial_force'][i]:16.4E}{pile['moment_x'][i]:16.4E}{pile['moment_y'][i]:16.4E}\n"
                )
    ...
    with open(filename, "w") as f:
        ...
        for pile in pile_results:
            ...
            for i in range(0, num_points, 6):
                ...
                line = "".join(
                    [f"{pile['z_coordinates'][j]:14.4E}" for j in range(i, end)]
                )
                f.write(line + "\n")
            ...
            for i in range(0, num_points, 6):
                ...
                line = "".join(
                    [f"{pile['axial_force'][j]:14.4E}" for j in range(i, end)]
                )
                f.write(line + "\n")
...
def write_header(f):
    ...
    f.write(
        "+    B     B     C    C    A     A    D    D        P           I      L     L   E          +\n"
    )
    f.write(
        "+    BBBBBB       CCCC     A     A    DDDDD   ===== P          III      LLLLL    EEEEEEE    +\n"
    )
    f.write("+" + " " * 78 + "+\n")
    f.write(
        "+                            Copyright 1990, Python Version 2023                           +\n"
    )
    f.write("+" + " " * 78 + "+\n")
    f.write("+" * 80 + "\n")
