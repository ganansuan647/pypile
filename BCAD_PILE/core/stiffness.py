...
def calculate_deformation_factors(pnum, zfr, zbl, pile_data):
    ...
    for k in range(pnum):
        for k1 in range(k+1, pnum):
            s = np.sqrt((gxy[k, 0] - gxy[k1, 0])**2 + (gxy[k, 1] - gxy[k1, 1])**2) - \
        ...
        if k > 0:
            ...
            for k1 in range(k):
                ...
                for ia in range(pile_data.nbl[k]):
                    ...
                    btx[k, ia] = (pile_data.pmt[k, ia] * bx1 / (pile_data.peh[k] * b))**0.2
                    bty[k, ia] = (pile_data.pmt[k, ia] * by1 / (pile_data.peh[k] * b))**0.2
        else:
...
def calculate_bottom_areas(pnum, zfr, zbl, pile_data):
    ...
    for k in range(pnum):
        if pile_data.ksu[k] > 2:
            ...
            for ia in range(pile_data.nbl[k]):
                w[k] += pile_data.hbl[k, ia] * (np.sin(ag) - pile_data.agl[k, 2] * 
...
def calculate_axial_stiffness(pnum, zfr, zbl, ao, pile_data):
    ...
    for k in range(1, pnum):
        ...
        for ia in range(k):
            if (pile_data.kctr[k] == pile_data.kctr[ia] and 
                abs(ao[k] - ao[ia]) < 1e-10):
                rzz[k] = rzz[ia]
                break
        else:
...
def calculate_lateral_stiffness(pnum, rzz, btx, bty, pile_data, element_stiffness):
    ...
    for k in range(pnum):
        if pile_data.nbl[k] == 0:
            ...
            h[pile_data.nbl[k]] = h[pile_data.nbl[k]-1] + pile_data.hbl[k, pile_data.nbl[k]-1]
