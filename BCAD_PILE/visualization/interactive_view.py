...
def create_interactive_visualization(vis_data):
    ...
    fig = make_subplots(
        ...
        subplot_titles=(
            ...
            "Pile Section Analysis",
            "3D View (Deformation Exaggerated)",
        ),
    ...
    fig.update_layout(
        ...
        width=1200,
        showlegend=True,
        legend=dict(orientation="h", yanchor="bottom", y=0.65, xanchor="right", x=0.95),
    )
...
def add_plan_view(fig, vis_data, row, col):
    ...
    fig.add_trace(
        go.Scatter(
            ...
            mode="markers+text",
            marker=dict(
                size=15, color="royalblue", line=dict(width=1, color="darkblue")
            ),
            text=text_labels,
...
def add_section_view(fig, vis_data, pile_index, row, col):
    ...
    # Update axes
    fig.update_xaxes(
        title="Displacement (m) / Moment (t·m) / Force (t)", row=row, col=col
    )
...
def add_3d_view(fig, vis_data, row, col):
    ...
    for i, pile in enumerate(all_piles):
        ...
        fig.add_trace(
            go.Scatter3d(
                ...
                mode="lines",
                line=dict(
                    color=deform, colorscale="Jet", width=5, cmin=0, cmax=max_deform
                ),
    ...
    xmin, xmax = (
        min([p["x"] for p in pile_positions]),
        max([p["x"] for p in pile_positions]),
    )
    ymin, ymax = (
        min([p["y"] for p in pile_positions]),
        max([p["y"] for p in pile_positions]),
    )
    ...
    # Set axis properties
    fig.update_scenes(
        xaxis=dict(title="X (m)", range=[xmin - x_padding, xmax + x_padding]),
        yaxis=dict(title="Y (m)", range=[ymin - y_padding, ymax + y_padding]),
        zaxis=dict(
            title="Z (m)",
