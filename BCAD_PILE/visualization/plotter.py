    ...
    def on_click(event):
        if event.inaxes == ax1:
            ...
            for i, pos in enumerate(pile_positions):
                dist = np.sqrt(
                    (event.xdata - pos["x"]) ** 2 + (event.ydata - pos["y"]) ** 2
                )
                if dist < min_dist:
                    min_dist = dist
                    closest_pile = i
...
def plot_pile_section(ax, vis_data, pile_index):
    ...
    ax.plot(
        np.array(mx) * moment_scale, z_coords, "r-", label="MX × " + str(moment_scale)
    )
    ax.plot(
        np.array(my) * moment_scale, z_coords, "c-", label="MY × " + str(moment_scale)
    )

    # Plot axial force
    ax.plot(
        np.array(nz) * axial_scale, z_coords, "k-", label="NZ × " + str(axial_scale)
    )
