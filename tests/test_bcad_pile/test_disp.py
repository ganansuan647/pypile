...
class TestDISP(unittest.TestCase):
    ...
    def test_disp_simple_case(self) -> None:
        ...
        # 使用Python实现计算位移
        duk_py = self.py_disp_simplified(
            jctr, ino, pnum, snum, pxy, sxy, agl, force, duk.copy(), so.copy(), esp
        )
        ...
        for i in range(6):
            ...
            self.assertEqual(
                actual_sign, expected_sign, f"位移分量 {i + 1} 的符号应与荷载符号一致"
            )

    def py_disp_simplified(
        self,
        jctr: int,
        ino: int,
        pnum: int,
    ...
    def py_disp_full(
        ...
        # 此实现暂时与简化版相同
        return self.py_disp_simplified(
            jctr, ino, pnum, snum, pxy, sxy, agl, force, duk, so, np.zeros((1000000, 6))
        )
