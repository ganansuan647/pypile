...
class TestBCAD_PILE(unittest.TestCase):
    ...
    def test_bcad_pile_main_functionality(self) -> None:
        ...
        so, disp, rzz = self.py_bcad_pile_simplified(
            pnum, pxy, agl, zfr, zbl, kctr, ksh, ksu,
            nfr, hfr, dof, nsf, nbl, hbl, dob, pmt, pfi, nsg,
            pmb, peh, pke, force
        )
        ...
        self.assertAlmostEqual(rzz[0], rzz[1], delta=1.0, 
                             msg="对称桩的轴向刚度应该近似相等")
    
    def py_bcad_pile_simplified(self, 
                               pnum: int,
                               pxy: np.ndarray,
                               agl: np.ndarray,
                               zfr: np.ndarray,
                               ...
                               peh: np.ndarray,
                               pke: np.ndarray,
                               force: np.ndarray) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        """
    ...
    def py_bcad_pile_full(self, 
                         so: np.ndarray, 
                         force: np.ndarray, 
                         jctr: int, 
                         ino: int) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        """
